...
class BartMultiHead(BartForConditionalGeneration):
    ...
    def forward(
        ...
        y_seqs=None, # this is the label sequence for the generation task
        labels=None,
        use_cache=None,
    ...
    def clf_forward(
        ...
        # NOTE: this is automatically done in self.model() but we explicity do it here
        # so we can pass it through to just self.decoder() for the clf task
        if decoder_input_ids is None and decoder_inputs_embeds is None:
            decoder_input_ids = shift_tokens_right(
        ...
        clf_decoder_outputs = self.model.decoder(
            ...
            encoder_hidden_states=encoder_outputs, #outputs.encoder_hidden_states,
